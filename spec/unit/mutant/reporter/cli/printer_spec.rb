RSpec.describe Mutant::Reporter::CLI::Printer, mutant_expression: 'Mutant::Reporter::CLI' do
  let(:output) { StringIO.new }

  subject { class_under_test.run(output, reportable) }

  def self.it_reports(expectation)
    it 'writes expected report' do
      allow(output).to receive(:tty?).and_return(tty?)
      expect(subject).to be(class_under_test)
      output.rewind
  ...
  context '#visit_collection' do
    let(:class_under_test) do
      nested_reporter = nested_reporter()
      Class.new(described_class) do
        define_method(:run) do
          visit_collection(nested_reporter, %w[foo bar])
        end
      end
    end
